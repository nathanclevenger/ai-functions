export const schema = (propDescriptions) => {
  ...
  if (Array.isArray(propDescriptions)) {
    ...
    const properties = Object.entries(propDescriptions).reduce(
      (acc, [key, value]) => {
        ...
        if (Array.isArray(value)) {
          ...
          if (itemType == 'string') {
            if (itemValue.includes('|')) {
              acc[key] = {
                type: 'string',
                enum: itemValue.split('|').map((value) => value.trim()),
              }
            } else {
              acc[key] = {
                type: 'array',
            ...
            // const { _description, itemSchema } = itemValue
            const description = itemValue._description
              ? `${itemValue._description}`
              : undefined
          ...
          if (type == 'string') {
            if (value.includes('|')) {
              acc[key] = {
                type: 'string',
                enum: value.split('|').map((value) => value.trim()),
              }
            } else {
