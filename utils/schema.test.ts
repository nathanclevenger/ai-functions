...
describe('parseStringDescription', () => {
  it('should create a string schema for plain strings', () => {
    ...
    expect(result).toEqual({ type: 'string', description: 'A plain string description' });
  });

  it('should create a string schema with enum for piped strings', () => {
    const result = parseStringDescription('option1 | option2 | option3');
    expect(result).toEqual({ type: 'string', enum: ['option1', 'option2', 'option3'] });
  });
  ...
  it('should create a number schema when prefixed with "number: "', () => {
    ...
    expect(result).toEqual({ type: 'number', description: 'A number description' });
  });
  ...
  it('should create a boolean schema when prefixed with "boolean: "', () => {
    ...
    expect(result).toEqual({ type: 'boolean', description: 'A boolean description' });
  });
});
  ...
  it('should create a complex object schema', () => {
    ...
    const expected = {
      ...
      properties: {
        ...
        isActive: { type: 'boolean', description: 'Whether the person is active or not' },
      },
