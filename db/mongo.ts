...
export const AIDB = (args: AIDBConfig | MongoClient) => {
  const config = (args as AIDBConfig).client
    ? (args as AIDBConfig)
    : { client: args as MongoClient }
  client = config.client
  db =
    typeof config.db === 'string'
      ? client.db(config.db)
      : config.db ?? client.db()
  cache =
    typeof config.cache === 'string'
      ? db.collection(config.cache)
      : config.cache ?? db.collection('ai-cache')
  cacheTTL = config.cacheTTL ?? 1000 * 60 * 60 * 24 * 30
  events =
    typeof config.events === 'string'
      ? db.collection(config.events)
      : config.events ?? db.collection('ai-events')
  jobs =
    typeof config.jobs === 'string'
      ? db.collection(config.jobs)
      : config.jobs ?? db.collection('ai-jobs')
